...
class Store {
    ...
    get isLogged() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_LOGGED_KEY], r => resolve(r[STORE_LOGGED_KEY]))
        })
    }

    get user() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_USER_KEY], r => resolve(r[STORE_USER_KEY]))
        })
    }

    get accessToken() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_ACCESS_TOKEN_KEY], r => resolve(r[STORE_ACCESS_TOKEN_KEY]))
        })
    }

    get config() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_CONFIG_KEY], r => resolve(r[STORE_CONFIG_KEY]))
        })
    }

    get stream() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_STREAM_KEY], r => resolve(r[STORE_STREAM_KEY]))
        })
    }

    get showNotification() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_SHOW_NOTIFICATION_KEY], r => resolve(r[STORE_SHOW_NOTIFICATION_KEY]))
        })
    }

    get lastNotifiedAt() {
        return new Promise((resolve) => {
            browser.storage.local.get([STORE_LAST_NOTIFIED_AT_KEY], r => resolve(r[STORE_LAST_NOTIFIED_AT_KEY]))
        })
    }

    set [STORE_USER_KEY](user) {
        browser.storage.local.set({ [STORE_USER_KEY]: user })
    }

    set [STORE_STREAM_KEY](stream) {
        browser.storage.local.set({ [STORE_STREAM_KEY]: stream })
    }

    set [STORE_LOGGED_KEY](status) {
        browser.storage.local.set({ [STORE_LOGGED_KEY]: status })
    }

    set [STORE_ACCESS_TOKEN_KEY](token) {
        browser.storage.local.set({ [STORE_ACCESS_TOKEN_KEY]: token })
    }

    set [STORE_SHOW_NOTIFICATION_KEY](status) {
        browser.storage.local.set({ [STORE_SHOW_NOTIFICATION_KEY]: status })
    }

    set [STORE_CONFIG_KEY](config) {
        browser.storage.local.set({ [STORE_CONFIG_KEY]: config })
    }

    set [STORE_LAST_NOTIFIED_AT_KEY](date) {
        browser.storage.local.set({ [STORE_LAST_NOTIFIED_AT_KEY]: date })
    }
}
